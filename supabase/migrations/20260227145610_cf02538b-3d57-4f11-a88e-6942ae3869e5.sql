...
CREATE POLICY "Users can view own documents storage" ON storage.objects FOR SELECT USING (bucket_id = 'tax-documents' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "Users can delete own documents storage" ON storage.objects FOR DELETE USING (bucket_id = 'tax-documents' AND (storage.foldername(name))[1] = auth.uid()::text);
